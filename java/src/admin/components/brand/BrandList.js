...
function BrandList() {
    ...
    const fetchBrands = async () => {
        try {
            const response = await index('brand');
            const responseData = response.data;
            if (responseData && Array.isArray(responseData)) {
                setBrands(responseData);
                const total = TotalBrands(responseData);
    ...
    const fetchTrashBrands = async () => {
        try {
            const response = await trash('brand/trash');
            const responseData = response.data;
            if (responseData && Array.isArray(responseData)) {
    ...
    const changeStatus = async (brandId) => {
        try {
            await status('brand/status', brandId);
            fetchBrands();
        } catch (error) {
            console.error("Lỗi khi gọi API lấy dữ liệu: ", error);
        }
    ...
    const restoreBrand = async (id) => {
        try {
            await restore('brand/restore', id);
            fetchBrands();
        } catch (error) {
            console.error("Lỗi khi gọi API lấy dữ liệu: ", error);
        }
    ...
    useEffect(() => {
        showTrash ? fetchTrashBrands() : fetchBrands();
    }, [showTrash]);
    ...
    return (
        <div className='mb-20'>
                ...
                <div className="w-full mb-1">
                    <div className="mb-4">
                                ...
                                <li>
                                    <div className="flex items-center">
                                        ...
                                        <span className="ml-1 text-gray-400 md:ml-2 dark:text-gray-500" aria-current="page">
                                            {showTrash ? 'Thùng rác' : 'Thương hiệu'}
                                        </span>
                                    </div>
                                </li>
                            </ol>
                        ...
                        <h1 className="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">
                            {showTrash ? 'Danh sách thùng rác' : 'Danh mục thương hiệu'}
                        </h1>
                    </div>
                            ...
                            <table className="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                                ...
                                <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                    ...
                                    {brands && brands.length > 0 && brands.map((brand, index) => {
                                        return (
                                            <BrandItem key={index} brand={brand} setEdit={setEdit} setDelete={setDelete} changeStatus={changeStatus} restoreBrand={restoreBrand} />
                                        );
                                    })} */}

                                    {filteredBrands.length === 0 ? (
                                        <li className="text-center py-4">Không có thương hiệu nào!</li>
                                    ) : (
                                        filteredBrands.map(brand => (
                                            <BrandItem key={index} brand={brand} setEdit={setEdit} setDelete={setDelete} changeStatus={changeStatus} restoreBrand={restoreBrand} deleteBrand={deleteBrand} />
                                        ))
                                    )}
            ...
            <div className="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
                <div className="flex items-center mb-4 sm:mb-0">
                    <span className="text-sm font-normal text-gray-500 dark:text-gray-400">Có <span className="font-semibold text-gray-900 dark:text-white">1-20</span> of <span className="font-semibold text-gray-900 dark:text-white">{totalBrands}</span></span>
                </div>
